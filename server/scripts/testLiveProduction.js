...
async function runAudit() {
  const randomDaysOffset = Math.floor(Math.random() * 300) + 30;
  const bookingDate = new Date(Date.now() + randomDaysOffset * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  const rescheduleDate = new Date(Date.now() + (randomDaysOffset + 1) * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
